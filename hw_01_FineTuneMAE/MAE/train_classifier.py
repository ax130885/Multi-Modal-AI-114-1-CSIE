...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str, default=None, help='TensorBoard 日誌目錄 (若為 None 則根據是否使用預訓練模型自動生成)')
    ...
    # 根據是否使用預訓練模型和模型路徑生成獨立的日誌目錄
    if args.log_dir is None:
        if args.pretrained_model_path is not None:
            # 有預訓練模型:從模型路徑提取實驗名稱
            model_name = os.path.splitext(os.path.basename(args.pretrained_model_path))[0]  # 提取檔名(不含副檔名)
            # 如果是消融實驗的模型 (包含 depth 或 width),生成對應的日誌目錄
            if 'depth' in model_name or 'width' in model_name:
                log_dir = os.path.join('logs', 'cifar10', 'ablation', f'cls_{model_name}')
            else:
                log_dir = os.path.join('logs', 'cifar10', 'pretrain-cls')
        else:
            # 從頭訓練
            log_dir = os.path.join('logs', 'cifar10', 'scratch-cls')
    else:
    ...
    for e in range(args.total_epoch):
        ...
        writer.add_scalar('tr/ce_loss', avg_train_loss, global_step=e)
        writer.add_scalar('tr/acc', avg_train_acc, global_step=e)
        writer.add_scalar('val/ce_loss', avg_val_loss, global_step=e)
        writer.add_scalar('val/acc', avg_val_acc, global_step=e)
